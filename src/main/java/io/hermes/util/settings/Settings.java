 ...
 *
 * <p>
 * Using {@link ImmutableSettings#settingsBuilder()} in order to create a builder which in turn can
 * create an immutable implementation of settings.
 *
 * @author spancer.ray
...
public interface Settings {
  ...
  /**
   * Component settings for a specific component. Returns all the settings for the given class,
   * where the FQN of the class is used, without the <tt>io.hermes<tt> prefix.
   */
  Settings getComponentSettings(Class component);

  /**
   * Component settings for a specific component. Returns all the settings for the given class,
   * where the FQN of the class is used, without provided prefix.
   */
  ...
  /**
   * Returns the setting value associated with the setting key. If it does not exists, returns the
   ...
   * @throws SettingsException Failure to parse the setting
   * @see TimeValue#parseTimeValue(String, io.hermes.util.TimeValue)
   */
   ...
   */
  <T> Class<? extends T> getAsClass(String setting, Class<? extends T> defaultClazz)
      throws NoClassSettingsException;

  /**
   * Returns the setting value (as a class) associated with the setting key. If the value itself
   * fails to represent a loadable class, the value will be appended to the <tt>prefixPackage</tt>
   * and suffixed with the <tt>suffixClassName</tt> and it will try to be loaded with it.
   *
   * @param setting         The setting key
   * @param defaultClazz    The class to return if no value is associated with the setting
   * @param prefixPackage   The prefix package to prefix the value with if failing to load the class
   *                        as is
   * @param suffixClassName The suffix class name to prefix the value with if failing to load the
   *                        class as is
   * @param <T>             The type of the class
   * @return The class represented by the setting value, or the default class provided if no value
   * exists
  ...
  /**
   * The values associated with a setting prefix as an array. The settings array is in the format
   * of: <tt>settingPrefix.[index]</tt>.
   *
