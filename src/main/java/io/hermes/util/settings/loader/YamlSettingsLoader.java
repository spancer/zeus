...
import io.hermes.util.io.FastByteArrayInputStream;
...
public class YamlSettingsLoader implements SettingsLoader {
  ...
  public Map<String, String> load(String source) throws IOException {
    // replace tabs with whitespace (yaml does not accept tabs, but many users might use it
    // still...)
    source = source.replace("\t", "  ");
    Yaml yaml = new Yaml();
    Map<Object, Object> yamlMap = yaml.load(source);
  ...
  public Map<String, String> load(byte[] source) throws IOException {
    ...
    Map<Object, Object> yamlMap =
        yaml.load(new FastByteArrayInputStream(source));
    StringBuilder sb = new StringBuilder();
    Map<String, String> settings = newHashMap();
    if (yamlMap == null) {
      return settings;
  ...
  private void serializeList(Map<String, String> settings, StringBuilder sb, List<String> path,
      List yamlList) {
    int counter = 0;
