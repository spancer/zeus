...
/**
 * A {@link Runnable} that changes the current thread name and reverts it back when its execution
 * ends. To change the default thread names set by Netty, use {@link
...
public class ThreadRenamingRunnable implements Runnable {
  ...
  private static final HermesLogger logger = Loggers.getLogger(ThreadRenamingRunnable.class);
  ...
  /**
   * Creates a new instance which wraps the specified {@code runnable} and changes the thread name
   * to the specified thread name when the specified {@code runnable} is running.
   */
  ...
  /**
   * Returns the {@link ThreadNameDeterminer} which overrides the proposed new thread name.
   */
  ...
  /**
   * Sets the {@link ThreadNameDeterminer} which overrides the proposed new thread name. Please note
   * that the specified {@link ThreadNameDeterminer} affects only new {@link
   * ThreadRenamingRunnable}s; the existing instances are not affected at all. Therefore, you should
   * make sure to call this method at the earliest possible point (i.e. before any Netty worker
   * thread starts) for consistent thread naming. Otherwise, you might see the default thread names
   * and the new names appear at the same time in the full thread dump.
   */
  ...
  private String getNewThreadName(String currentThreadName) {
    ...
    try {
      newThreadName =
          getThreadNameDeterminer().determineThreadName(currentThreadName, proposedThreadName);
    } catch (Throwable t) {
