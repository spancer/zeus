...
import io.hermes.HermesNullPointerException;
 ...
 * you expect to be <i>true</i>, just as you do with an
 * <a href="http://java.sun.com/j2se/1.5.0/docs/guide/language/assert.html"> {@code assert}</a> or
 * a JUnit {@code assertTrue()} call.
 * <p/>
 * <p>
 * Take care not to confuse precondition checking with other similar types of checks! Precondition
 * exceptions -- including those provided here, but also {@link IndexOutOfBoundsException}, {@link
 * NoSuchElementException}, {@link UnsupportedOperationException} and others -- are used to signal
 * that the <i>calling method</i> has made an error. This tells the caller that it should not have
 * invoked the method when it did, with the arguments it did, or perhaps <i>ever</i>. Postcondition
 * or other invariant failures should not throw these types of exceptions.
 * <p/>
 * <p>
 * <b>Note:</b> The methods of the {@code Preconditions} class are highly unusual in one way: they
 * are <i>supposed to</i> throw exceptions, and promise in their specifications to do so even when
 * given perfectly valid input. That is, {@code null} is a valid parameter to the method {@link
 * #checkNotNull(Object)} -- and technically this parameter could be even marked as Nullable -- yet
 * the method will still throw an exception anyway, because that's what its contract says to do.
 *
 * @author spancer.ray
 */
public final class Preconditions {
   ...
   * @throws io.hermes.HermesIllegalArgumentException if {@code expression} is false
   */
  ...
  public static void checkArgument(boolean expression, Object errorMessage) {
    if (!expression) {
      throw new HermesIllegalArgumentException(String.valueOf(errorMessage));
    }
  }
   ...
   * @throws io.hermes.HermesIllegalArgumentException if {@code expression} is false
   * @throws io.hermes.HermesNullPointerException     if the check fails and either {@code
   *                                                  errorMessageTemplate} or {@code
   *                                                  errorMessageArgs} is null (don't let this
   *                                                  happen)
   */
  public static void checkArgument(boolean expression, String errorMessageTemplate,
      Object... errorMessageArgs) {
    if (!expression) {
      throw new HermesIllegalArgumentException(format(errorMessageTemplate, errorMessageArgs));
    }
  }
   ...
   *
   * @param expression a boolean expression
   * @throws io.hermes.HermesIllegalStateException if {@code expression} is false
   */
  ...
  public static void checkState(boolean expression, Object errorMessage) {
    if (!expression) {
      throw new HermesIllegalStateException(String.valueOf(errorMessage));
    }
  }
   ...
   * @throws io.hermes.HermesIllegalStateException if {@code expression} is false
   * @throws io.hermes.HermesNullPointerException  if the check fails and either {@code
   *                                               errorMessageTemplate} or {@code errorMessageArgs}
   *                                               is null (don't let this happen)
   */
  public static void checkState(boolean expression, String errorMessageTemplate,
      Object... errorMessageArgs) {
    if (!expression) {
      throw new HermesIllegalStateException(format(errorMessageTemplate, errorMessageArgs));
    }
  }
   ...
   * @throws io.hermes.HermesNullPointerException if {@code reference} is null
   */
  public static <T> T checkNotNull(T reference) {
    if (reference == null) {
      throw new HermesNullPointerException();
    }
    return reference;
  }
  ...
  public static <T> T checkNotNull(T reference, Object errorMessage) {
    if (reference == null) {
      throw new HermesNullPointerException(String.valueOf(errorMessage));
    }
    return reference;
  }
  ...
  public static <T> T checkNotNull(T reference, String errorMessageTemplate,
    ...
    if (reference == null) {
      ...
      throw new HermesNullPointerException(format(errorMessageTemplate, errorMessageArgs));
    }
    return reference;
  }

  /**
   * Ensures that an {@code Iterable} object passed as a parameter to the calling method is not null
   ...
   * @throws io.hermes.HermesNullPointerException if {@code iterable} is null or contains at least
   *                                              one null element
   */
  ...
  private static boolean containsOrIsNull(Iterable<?> iterable) {
    ...
    if (iterable instanceof Collection) {
      ...
      } catch (HermesNullPointerException e) {
   ...
   *                                                  than {@code size}
   * @throws io.hermes.HermesIllegalArgumentException if {@code size} is negative
   */
   ...
   *                                                  {@code size}, or if {@code end} is less than
   *                                                  {@code start}
   * @throws io.hermes.HermesIllegalArgumentException if {@code size} is negative
   */
   ...
   *                 converted to strings using {@link String#valueOf(Object)}. Arguments can be
   *                 null.
   */
  // VisibleForTesting
