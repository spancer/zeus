...
public class ThreadLocals {
  ...
  private static final HermesLogger logger = Loggers.getLogger(ThreadLocals.class);
  ...
  private static void clearThreadLocalMap(Object map, Field internalTableField)
          ...
          if (table[j] != null) {
            ...
            if (remove) {
              ...
              if (logger.isDebugEnabled()) {
                logger.debug(
                    "ThreadLocal with key of type [{0}] (value [{1}]) and a value of type [{2}] (value [{3}]):  The ThreadLocal has been forcibly removed.",
                    args);
              }
              if (key == null) {
                staleEntriesCount++;
  ...
  /*
   * Clears the given thread local map object. Also pass in the field that points to the internal
   * table to save re-calculating it on every call to this method.
   */
