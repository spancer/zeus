...
/**
 * A global registry of all different types of {@link io.hermes.util.transport.TransportAddress}
 * allowing to perform serialization of them.
 *
 * <p>
 * By default, adds {@link io.hermes.util.transport.InetSocketTransportAddress}.
 *
 * @author spancer.ray
 */
public abstract class TransportAddressSerializers {

  private static final HermesLogger logger = Loggers.getLogger(TransportAddressSerializers.class);

  private static ImmutableMap<Short, Constructor<? extends TransportAddress>> addressConstructors =
      ImmutableMap.of();
  ...
  public static synchronized void addAddressType(TransportAddress address) throws Exception {
    if (addressConstructors.containsKey(address.uniqueAddressTypeId())) {
      throw new HermesIllegalStateException(
          "Address [" + address.uniqueAddressTypeId() + "] already bound");
    }
    Constructor<? extends TransportAddress> constructor =
        address.getClass().getDeclaredConstructor();
    constructor.setAccessible(true);
    addressConstructors = newMapBuilder(addressConstructors)
  ...
  public static void addressToStream(StreamOutput out, TransportAddress address)
      throws IOException {
