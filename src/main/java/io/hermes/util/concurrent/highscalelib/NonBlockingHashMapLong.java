...
/**
 * A lock-free alternate implementation of {@link java.util.ConcurrentHashMap} with
 * <strong>primitive long keys</strong>, better scaling properties and generally lower costs. The
 * use of {@code long} keys allows for faster compares and lower memory costs. The Map provides
 * identical correctness properties as ConcurrentHashMap. All operations are non-blocking and
 * multi-thread safe, including all update operations. {@link NonBlockingHashMapLong} scales
 * substatially better than {@link java.util.ConcurrentHashMap} for high update rates, even with a
 * large concurrency factor. Scaling is linear up to 768 CPUs on a 768-CPU Azul box, even with 100%
 * updates or 100% reads or any fraction in-between. Linear scaling up to all cpus has been observed
 * on a 32-way Sun US2 box, 32-way Sun Niagra box, 8-way Intel box and a 4-way Power box.
 * <p/>
 * <p>
 * <strong>The main benefit of this class</strong> over using plain
 * {@link org.cliffc.high_scale_lib.NonBlockingHashMap} with {@link Long} keys is that it avoids the
 * auto-boxing and unboxing costs. Since auto-boxing is <em>automatic</em>, it is easy to
 * accidentally cause auto-boxing and negate the space and speed benefits.
 * <p/>
 * <p>
 * This class obeys the same functional specification as {@link java.util.Hashtable}, and includes
 * versions of methods corresponding to each method of <tt>Hashtable</tt>. However, even though all
 * operations are thread-safe, operations do <em>not</em> entail locking and there is <em>not</em>
 * any support for locking the entire table in a way that prevents all access. This class is fully
 * interoperable with <tt>Hashtable</tt> in programs that rely on its thread safety but not on its
 * synchronization details.
 * <p/>
 * <p>
 * Operations (including <tt>put</tt>) generally do not block, so may overlap with other update
 * operations (including other <tt>puts</tt> and <tt>removes</tt>). Retrievals reflect the results
 * of the most recently <em>completed</em> update operations holding upon their onset. For aggregate
 * operations such as <tt>putAll</tt>, concurrent retrievals may reflect insertion or removal of
 * only some entries. Similarly, Iterators and Enumerations return elements reflecting the state of
 * the hash table at some point at or since the creation of the iterator/enumeration. They do
 * <em>not</em> throw {@link ConcurrentModificationException}. However, iterators are designed to
 * be used by only one thread at a time.
 * <p/>
 * <p>
 * Very full tables, or tables with high reprobe rates may trigger an internal resize operation to
 * move into a larger table. Resizing is not terribly expensive, but it is not free either; during
 * resize operations table throughput may drop somewhat. All threads that visit the table during a
 * resize will 'help' the resizing but will still be allowed to complete their operation before the
 * resize is finished (i.e., a simple 'get' operation on a million-entry table undergoing resizing
 * will not need to block until the entire million entries are copied).
 * <p/>
 * <p>
 ...
 * <tt>null</tt> to be used as a value.
 *
...
public class NonBlockingHashMapLong<TypeV> extends AbstractMap<Long, TypeV>
  ...
  /**
   * Create a new NonBlockingHashMapLong with default minimum size (currently set to 8 K/V pairs or
   * roughly 84 bytes on a standard 32-bit JVM).
   */
  ...
  /**
   * Create a new NonBlockingHashMapLong with initial room for the given number of elements, thus
   * avoiding internal resizing operations to reach an appropriate size. Large numbers here when
   * used with a small count of elements will sacrifice space for a small amount of time gained. The
  ...
  /**
   * Create a new NonBlockingHashMapLong, setting the space-for-speed tradeoff. {@code true}
   * optimizes for space and is the default. {@code false} optimizes for speed and doubles space
   * costs for roughly a 10% speed improvement.
   */
  ...
  /**
   * Create a new NonBlockingHashMapLong, setting both the initial size and the space-for-speed
   * tradeoff. {@code true} optimizes for space and is the default. {@code false} optimizes for
   * speed and doubles space costs for roughly a 10% speed improvement.
   */
  ...
  /**
   * Get and clear the current count of reprobes. Reprobes happen on key collisions, and a high
   * reprobe rate may indicate a poor hash function or weaknesses in the table resizing function.
   *
   * @return the count of reprobes since the last call to {@link #reprobes} or since the table was
   * created.
   */
   ...
   * <p>
   * The value can be retrieved by calling {@link #get} with a key that is equal to the original
   * key.
   *
  ...
  /**
   * Atomically, do a {@link #put} if-and-only-if the key is not mapped. Useful to ensure that only
   * a single mapping for the key exists, even if many threads are trying to create the mapping in
   * parallel.
   *
  ...
  /**
   * Atomically do a {@link #remove(long)} if-and-only-if the key is mapped to a value which is
   * <code>equals</code> to the given value.
   *
   * @throws NullPointerException if the specified value is null
   */
  ...
  /**
   * Atomically do a <code>put(key,val)</code> if-and-only-if the key is mapped to some value
   * already.
   *
   * @throws NullPointerException if the specified value is null
   */
  ...
  /**
   * Atomically do a <code>put(key,newValue)</code> if-and-only-if the key is mapped a value which
   * is <code>equals</code> to <code>oldValue</code>.
   *
   * @throws NullPointerException if the specified value is null
   */
  ...
  private final TypeV putIfMatch(long key, Object newVal, Object oldVal) {
    ...
    if (key == NO_KEY) {
      ...
      if (oldVal == NO_MATCH_OLD || // Do we care about expected-Value at all?
          ...
          (oldVal == MATCH_ANY && curVal != TOMBSTONE) || oldVal.equals(curVal)) // Expensive equals
      // check
      {
  ...
  /**
   * Returns <tt>true</tt> if this Map maps one or more keys to the specified value. <em>Note</em>:
   * This method requires a full internal traversal of the hash table and is much slower than {@link
   * #containsKey}.
   *
  ...
  /**
   * Returns a {@link Set} view of the keys contained in this map; with care the keys may be
   * iterated over <strong>without auto-boxing</strong>. The set is backed by the map, so changes to
   * the map are reflected in the set, and vice-versa. The set supports element removal, which
   ...
   * <p/>
   * <p>
   * <strong>Warning:</strong> the iterator associated with this Set requires the creation of
   * {@link java.util.Map.Entry} objects with each iteration. The {@link
   * org.cliffc.high_scale_lib.NonBlockingHashMap} does not normally create or using {@link
   * java.util.Map.Entry} objects so they will be created soley to support this iteration. Iterating
   * using {@link #keySet} or {@link #values} will be more efficient. In addition, this version
   * requires <strong>auto-boxing</strong> the keys.
   */
  ...
  private static final class CHM<TypeV> implements Serializable {
    ...
    private final Object get_impl(final long key) {
      ...
      while (true) {
        ...
        final Object V = _vals[idx]; // Get value before volatile read, could be null or Tombstone
        // or Prime
        if (K == NO_KEY) {
    ...
    private final CHM resize() {
      ...
      // System.out.println("old="+oldlen+" new="+newsz+" size()="+sz+" est_slots()="+q+"
      // millis="+(tm-_nbhml._last_resize_milli));
  ...
  /**
   * A class which implements the {@link Iterator} and {@link Enumeration} interfaces, generified to
   * the {@link Long} class and supporting a <strong>non-auto-boxing</strong> {@link #nextLong}
   * function.
   */
