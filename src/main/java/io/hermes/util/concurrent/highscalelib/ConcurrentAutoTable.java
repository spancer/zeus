...
/**
 * An auto-resizing table of {@code longs}, supporting low-contention CAS operations. Updates are
 * done with CAS's to no particular table element. The intent is to support highly scalable
 * counters, r/w locks, and other structures where the updates are associative, loss-free
 * (no-brainer), and otherwise happen at such a high volume that the cache contention for CAS'ing a
 * single word is unacceptable.
 * <p/>
 * <p>
 * This API is overkill for simple counters (e.g. no need for the 'mask') and is untested as an API
 * for making a scalable r/w lock and so is likely to change!
 *
 * @author Cliff Click
 * @since 1.5
 */
...
public class ConcurrentAutoTable implements Serializable {
  ...
  /**
   * Add the given value to current counter value. Concurrent updates will not be lost, but
   * addAndGet or getAndAdd are not implemented because the total counter value (i.e., {@link #get})
   * is not atomically updated. Updates are striped across an array of counters to avoid cache
   * contention and has been tested with performance scaling linearly up to 768 CPUs.
   */
  ...
  /**
   * Atomically set the sum of the striped counters to specified value. Rather more expensive than a
   * simple store, in order to remain atomic.
   */
  ...
  /**
   * Current value of the counter. Since other threads are updating furiously the value is only
   * approximate, but it includes all counts made by the current thread. Requires a pass over the
   * internally striped counters.
   */
  public long get() {
    return _cat.sum(0);
  }
  ...
  /**
   * A cheaper {@link #get}. Updated only once/millisecond, but as fast as a simple load instruction
   * when not updating.
   */
  ...
  /**
   * A more verbose print than {@link #toString}, showing internal structure. Useful for debugging.
   */
  public void print() {
    _cat.print();
  }
