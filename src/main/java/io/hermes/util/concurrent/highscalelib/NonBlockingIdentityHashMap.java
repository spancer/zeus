...
/**
 * A lock-free alternate implementation of {@link java.util.concurrent.ConcurrentHashMap} with
 * better scaling properties and generally lower costs to mutate the Map. It provides identical
 * correctness properties as ConcurrentHashMap. All operations are non-blocking and multi-thread
 * safe, including all update operations. {@link NonBlockingHashMap} scales substatially better than
 * {@link java.util.concurrent.ConcurrentHashMap} for high update rates, even with a large
 * concurrency factor. Scaling is linear up to 768 CPUs on a 768-CPU Azul box, even with 100%
 ...
 * @author Cliff Click
 * @author Prashant Deva Modified from original NonBlockingHashMap to use identity equality. Uses
 * System.identityHashCode() to calculate hashMap. Key equality is compared using '=='.
 * @since 1.5
 */
...
public class NonBlockingIdentityHashMap<TypeK, TypeV> extends AbstractMap<TypeK, TypeV>
  ...
  /**
   * Create a new NonBlockingHashMap with default minimum size (currently set to 8 K/V pairs or
  ...
  /**
   * Create a new NonBlockingHashMap with initial room for the given number of elements, thus
  ...
  private static final Object get_impl(final NonBlockingIdentityHashMap topmap, final Object[] kvs,
      final Object key, final int fullhash) {
    ...
    while (true) {
      ...
      final Object V = val(kvs, idx); // Get value before volatile read, could be null or Tombstone
      // or Prime
      if (K == null) {
          ...
          key == TOMBSTONE) // found a TOMBSTONE key, means no more keys in this table
      {
        return newkvs == null ? null
            : get_impl(topmap, topmap.help_copy(newkvs), key, fullhash); // Retry
      }
      // in
      // the
      // new
  ...
  private static final Object putIfMatch(final NonBlockingIdentityHashMap topmap,
      final Object[] kvs, final Object key, final Object putval, final Object expVal) {
    assert putval != null;
    ...
    if (newkvs != null) {
      return putIfMatch(topmap, chm.copy_slot_and_check(topmap, kvs, idx, expVal), key, putval,
          expVal);
    }
  ...
  /**
   * Atomically do a {@link #remove(Object)} if-and-only-if the key is mapped to a value which is
  ...
  /**
   * Copies all of the mappings from the specified map to this one, replacing any existing
   * mappings.
   *
  ...
  public Object clone() {
    try {
      ...
      NonBlockingIdentityHashMap<TypeK, TypeV> t =
          (NonBlockingIdentityHashMap<TypeK, TypeV>) super.clone();
   ...
   * {@link java.util.Map.Entry} objects with each iteration. The {@link NonBlockingIdentityHashMap}
   * does not normally create or using {@link java.util.Map.Entry} objects so they will be created
   * soley to support this iteration. Iterating using {@link #keySet} or {@link #values} will be
   * more efficient.
   */
  @Override
  ...
  private static final class CHM<TypeK, TypeV> {
    ...
    private final void help_copy_impl(NonBlockingIdentityHashMap topmap, Object[] oldkvs,
        boolean copy_all) {
      ...
      while (_copyDone < oldlen) { // Still needing to copy?
        ...
        for (int i = 0; i < MIN_COPY_WORK; i++) {
          if (copy_slot(topmap, (copyidx + i) & (oldlen - 1), oldkvs, newkvs)) // Made an oldtable
          // slot go dead?
          {
            workdone++; // Yes!
          }
    ...
    private final Object[] copy_slot_and_check(NonBlockingIdentityHashMap topmap, Object[] oldkvs,
        int idx, Object should_help) {
      assert chm(oldkvs) == this;
      Object[] newkvs = _newkvs; // VOLATILE READ
    ...
    private final void copy_check_and_promote(NonBlockingIdentityHashMap topmap, Object[] oldkvs,
        int workdone) {
      assert chm(oldkvs) == this;
    ...
    private boolean copy_slot(NonBlockingIdentityHashMap topmap, int idx, Object[] oldkvs,
        Object[] newkvs) {
