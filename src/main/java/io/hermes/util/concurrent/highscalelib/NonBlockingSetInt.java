...
/**
 * A multi-threaded bit-vector set, implemented as an array of primitive {@code longs}. All
 * operations are non-blocking and multi-threaded safe. {@link #contains(int)} calls are roughly the
 * same speed as a {load, mask} sequence. {@link #add(int)} and {@link #remove(int)} calls are a tad
 * more expensive than a {load, mask, store} sequence because they must use a CAS. The bit-vector is
 * auto-sizing.
 * <p/>
 * <p>
 * <em>General note of caution:</em> The Set API allows the use of {@link Integer} with silent
 * autoboxing - which can be very expensive if many calls are being made. Since autoboxing is silent
 * you may not be aware that this is going on. The built-in API takes lower-case {@code ints} and is
 * much more efficient.
 * <p/>
 * <p>
 * Space: space is used in proportion to the largest element, as opposed to the number of elements
 * (as is the case with hash-table based Set implementations). Space is approximately
 * (largest_element/8 + 64) bytes.
 * <p/>
...
public class NonBlockingSetInt extends AbstractSet<Integer> implements Serializable {
  ...
  /**
   * Add {@code i} to the set. Uppercase {@link Integer} version of add, requires auto-unboxing.
   * When possible use the {@code int} version of {@link #add(int)} for efficiency.
   *
  ...
  /**
   * Test if {@code o} is in the set. This is the uppercase {@link Integer} version of contains,
   * requires a type-check and auto-unboxing. When possible use the {@code int} version of {@link
   * #contains(int)} for efficiency.
   *
  ...
  /**
   * Remove {@code o} from the set. This is the uppercase {@link Integer} version of remove,
   * requires a type-check and auto-unboxing. When possible use the {@code int} version of {@link
   * #remove(int)} for efficiency.
   *
  ...
  /**
   * Add {@code i} to the set. This is the lower-case '{@code int}' version of {@link #add} - no
   * autoboxing. Negative values throw IllegalArgumentException.
   *
   * @return <tt>true</tt> if i was added to the set.
   * @throws IllegalArgumentException if i is negative.
   */
  ...
  /**
   * Test if {@code i} is in the set. This is the lower-case '{@code int}' version of {@link
   * #contains} - no autoboxing.
   *
  ...
  /**
   * Remove {@code i} from the set. This is the fast lower-case '{@code int}' version of {@link
   * #remove} - no autoboxing.
   *
   * @return <tt>true</tt> if i was added to the set.
   */
  ...
  /**
   * Current count of elements in the set. Due to concurrent racing updates, the size is only ever
   * approximate. Updates due to the calling thread are immediately visible to calling thread.
   *
  ...
  /**
   * Standard Java {@link Iterator}. Not very efficient because it auto-boxes the returned values.
   */
  ...
  private static final class NBSI {
    ...
    private transient final AtomicInteger _copyDone; // Used to count words copied in a resize
    // operation
