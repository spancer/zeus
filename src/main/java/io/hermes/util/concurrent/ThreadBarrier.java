 ...
 * barrier point. Barriers are useful in programs involving a fixed sized party of threads that must
 * occasionally wait for each other. <code>ThreadBarrier</code> adds a <i>cause</i> to {@link
 * BrokenBarrierException} thrown by a {@link #reset()} operation defined by {@link CyclicBarrier}.
 * <p/>
 * <p/>
 * <b>Sample usage:</b> <br>
...
public class ThreadBarrier extends CyclicBarrier {
  ...
  /**
   * The cause of a {@link BrokenBarrierException} and {@link TimeoutException} thrown from an
   * await() when {@link #reset(Throwable)} was invoked.
   */
   ...
   * they will return with a {@link BrokenBarrierException}. Note that resets <em>after</em> a
   * breakage has occurred for other reasons can be complicated to carry out; threads need to
   * re-synchronize in some other way, and choose one to perform the reset. It may be preferable to
  ...
  /**
   * Queries if this barrier is in a broken state. Note that if {@link #reset(Throwable)} is invoked
   * the barrier will remain broken, while {@link #reset()} will reset the barrier to its initial
   * state and {@link #isBroken()} will return false.
   *
   * @return {@code true} if one or more parties broke out of this barrier due to interruption or
   * timeout since construction or the last reset, or a barrier action failed due to an exception;
  ...
  /**
   * Inspects if the barrier is broken. If for any reason, the barrier was broken, a {@link
   ...
   * <p/>
   * Note: This call is not atomic in respect to await/reset calls. A breakIfBroken() may be context
   * switched to invoke a reset() prior to await(). This resets the barrier to its initial state -
   * parties not currently waiting at the barrier will not be accounted for! An await that wasn't
   * time limited, will block indefinitely.
   *
  ...
  /**
   * Initializes the cause of this throwable to the specified value. The cause is the throwable that
   * was initialized by {@link #reset(Throwable)}.
   *
   * @param t throwable.
   */
  ...
  /**
   * A Barrier action to be used in conjunction with {@link ThreadBarrier} to measure performance
   * between barrier awaits. This runnable will execute when the barrier is tripped. Make sure to
   * reset() the timer before next Measurement.
   *
