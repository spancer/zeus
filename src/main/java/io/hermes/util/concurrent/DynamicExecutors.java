...
public class DynamicExecutors {
   ...
   * threads when they are available. Calls to <tt>execute</tt> will reuse previously constructed
   * threads if available. If no existing thread is available, a new thread will be created and
   * added to the pool. No more than <tt>max</tt> threads will be created. Threads that have not
   * been used for a <tt>keepAlive</tt> timeout are terminated and removed from the cache. Thus, a
   * pool that remains idle for long enough will not consume any resources other than the
   * <tt>min</tt> specified.
   *
  ...
  public static ExecutorService newScalingThreadPool(int min, int max, long keepAliveTime,
    ...
    DynamicThreadPoolExecutor.DynamicQueue<Runnable> queue =
        new DynamicThreadPoolExecutor.DynamicQueue<Runnable>();
  ...
  public static ExecutorService newBlockingThreadPool(int min, int max, long keepAliveTime,
    ...
    return newBlockingThreadPool(min, max, keepAliveTime, capacity, waitTime,
  ...
  public static ExecutorService newBlockingThreadPool(int min, int max, long keepAliveTime,
    ...
    DynamicThreadPoolExecutor.DynamicQueue<Runnable> queue =
        new DynamicThreadPoolExecutor.DynamicQueue<Runnable>(capacity);
    ThreadPoolExecutor executor = new DynamicThreadPoolExecutor(min, max, keepAliveTime,
        TimeUnit.MILLISECONDS, queue, threadFactory);
    executor
        .setRejectedExecutionHandler(new DynamicThreadPoolExecutor.TimedBlockingPolicy(waitTime));
    queue.setThreadPoolExecutor(executor);
    return executor;
  }
  ...
  public static ThreadFactory daemonThreadFactory(String namePrefix) {
    ...
    return new ThreadFactory() {
      public Thread newThread(Runnable r) {
        ...
        /*
         * Thread name: owner-pool-N-thread-M, where N is the sequence number of this factory, and M
