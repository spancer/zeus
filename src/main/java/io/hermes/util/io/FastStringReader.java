...
public class FastStringReader extends CharSequenceReader {
   ...
   * {@link Reader} superclass throws an exception in this case. Negative values of <code>ns</code>
   * cause the stream to skip backwards. Negative return values indicate a skip backwards. It is not
   ...
   *                       the mark. Because the stream's input comes from a string, there is no
   *                       actual limit, so this argument must not be negative, but is otherwise
   *                       ignored.
