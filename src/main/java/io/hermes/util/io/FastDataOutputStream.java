...
public class FastDataOutputStream extends FilterOutputStream implements DataOutput {
  ...
  /**
   * The number of bytes written to the data output stream so far. If this counter overflows, it
   * will be wrapped to Integer.MAX_VALUE.
   */
   ...
   * manner.
   * <p/>
   * First, two bytes are written to out as if by the <code>writeShort</code> method giving the
   ...
   * <code>false</code> is written out as the value <code>(byte)0</code>. If no exception is
  ...
  /**
   * Writes out a <code>byte</code> to the underlying output stream as a 1-byte value. If no
   * exception is thrown, the counter <code>written</code> is incremented by <code>1</code>.
   *
  ...
  /**
   * Writes a <code>short</code> to the underlying output stream as two bytes, high byte first. If
   ...
   * If no exception is thrown, the counter <code>written</code> is incremented by <code>2</code>.
   *
  ...
  /**
   * Writes a <code>long</code> to the underlying output stream as eight bytes, high byte first. In
   * no exception is thrown, the counter <code>written</code> is incremented by <code>8</code>.
   *
   ...
   * in class <code>Float</code>, and then writes that <code>int</code> value to the underlying
   * output stream as a 4-byte quantity, high byte first. If no exception is thrown, the counter
   ...
   * the string is written out, in sequence, by discarding its high eight bits. If no exception is
   ...
   * written to the data output stream as if by the <code>writeChar</code> method. If no exception
   * is thrown, the counter <code>written</code> is incremented by twice the length of
   * <code>s</code>.
   *
   ...
   * giving the number of bytes to follow. This value is the number of bytes actually written out,
   * not the length of the string. Following the length, each character of the string is output, in
   * sequence, using the modified UTF-8 encoding for the character. If no exception is thrown, the
   * counter <code>written</code> is incremented by the total number of bytes written to the output
   * stream. This will be at least two plus the length of <code>str</code>, and at most two plus
   * thrice the length of <code>str</code>.
