...
/**
 * A similar class to {@link java.io.CharArrayWriter} allowing to get the underlying <tt>char[]</tt>
 * buffer.
 *
 * @author spancer.ray
 */
...
public class FastCharArrayWriter extends Writer {
   ...
   * the entire sequence may not be appended. For instance, invoking the <tt>toString</tt> method of
   * a character buffer will return a subsequence whose content depends upon the buffer's position
   * and limit.
   *
   ...
   * <p/>
   * <p>
   * An invocation of this method of the form <tt>out.append(csq, start, end)</tt> when <tt>csq</tt>
   * is not <tt>null</tt>, behaves in exactly the same way as the invocation
   * <p/>
   *
   * <pre>
   ...
   *              is
   *              <tt>null</tt>, then characters will be appended as if <tt>csq</tt> contained the
   *              four characters <tt>"null"</tt>.
   ...
   * <p/>
   * <p>
   * An invocation of this method of the form <tt>out.append(c)</tt> behaves in exactly the same way
   * as the invocation
   * <p/>
   *
   * <pre>
  ...
  /**
   * Returns the underlying char array. Note, use {@link #size()} in order to know the size of of
   * the actual content within the array.
   */
  ...
  public static class Cached {
    ...
    private static final ThreadLocal<ThreadLocals.CleanableValue<FastCharArrayWriter>> cache =
        new ThreadLocal<ThreadLocals.CleanableValue<FastCharArrayWriter>>() {
          @Override
          protected ThreadLocals.CleanableValue<FastCharArrayWriter> initialValue() {
