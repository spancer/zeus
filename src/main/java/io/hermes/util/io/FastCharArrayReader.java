...
public class FastCharArrayReader extends Reader {
   ...
   *
   * <p>
   * The resulting reader will start reading at the given <tt>offset</tt>. The total number of
   * <tt>char</tt> values that can be read from this reader will be either <tt>length</tt> or
   ...
   *
   * <p>
   * The <code>n</code> parameter may be negative, even though the <code>skip</code> method of the
   * {@link Reader} superclass throws an exception in this case. If <code>n</code> is negative, then
  ...
  /**
   * Tells whether this stream is ready to be read. Character-array readers are always ready to be
   * read.
   *
   * @throws IOException If an I/O error occurs
   */
   ...
   *                       the mark. Because the stream's input comes from a character array, there
   *                       is no actual limit; hence this argument is ignored.
   * @throws IOException If an I/O error occurs
   */
