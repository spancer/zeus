...
public class TIntDoubleHashMap extends TIntHash implements Externalizable {
  ...
  private final TIntDoubleProcedure PUT_ALL_PROC = new TIntDoubleProcedure() {
  ...
  /**
   * Creates a new <code>TIntDoubleHashMap</code> instance with the default capacity and load
   * factor.
   */
  ...
  /**
   * Creates a new <code>TIntDoubleHashMap</code> instance with a prime capacity equal to or greater
  ...
  /**
   * Creates a new <code>TIntDoubleHashMap</code> instance whose capacity is the next highest prime
  ...
  /**
   * Creates a new <code>TIntDoubleHashMap</code> instance with a prime value at or near the
   ...
   * @return the previous value associated with <tt>key</tt>, or (int)0 if none was found.
   */
   ...
   */
  public void putAll(TIntDoubleHashMap map) {
  ...
  public int[] keys(int[] a) {
    int size = size();
    if (a.length < size) {
      a = (int[]) java.lang.reflect.Array.newInstance(a.getClass().getComponentType(), size);
    }
   ...
   * @since 2.0b1
   */
  public double adjustOrPutValue(final int key, final double adjust_amount,
