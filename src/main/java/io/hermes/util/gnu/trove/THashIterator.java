 ...
 * <p/>
 * <p>
 * Note that iteration is fastest if you forego the calls to <tt>hasNext</tt> in favor of checking
...
abstract class THashIterator<V> extends TIterator implements Iterator<V> {
   ...
   * @throws ConcurrentModificationException if the structure was changed using a method that isn't
   *                                         on this iterator.
   * @throws NoSuchElementException          if this is called on an exhausted iterator.
   */
  public V next() {
    moveToNextIndex();
   ...
   * @throws ConcurrentModificationException if the underlying collection's size has been modified
  ...
  /**
   * Returns the object at the specified index. Subclasses should implement this to return the
   * appropriate object for the given index.
   *
