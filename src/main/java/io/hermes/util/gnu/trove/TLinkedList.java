 ...
 * <p/>
 * <p>
 * Using this implementation allows you to get java.util.LinkedList behavior (a doubly linked list,
 * with Iterators that support insert and delete operations) without incurring the overhead of
 * creating <tt>Node</tt> wrapper objects for every element in your list.
 * </p>
 * <p/>
 * <p>
 * The requirement to achieve this time/space gain is that the Objects stored in the List implement
 * the <tt>TLinkable</tt> interface.
 * </p>
 * <p/>
 * <p>
 * The limitations are that you cannot put the same object into more than one list or more than once
 * in the same list. You must also ensure that you only remove objects that are actually in the
 * list. That is, if you have an object A and lists l1 and l2, you must ensure that you invoke
 * List.remove(A) on the correct list. It is also forbidden to invoke List.remove() with an
 * unaffiliated TLinkable (one that belongs to no list): this will destroy the list you invoke it
 * on.
 * </p>
 * <p/>
 * <p>
 ...
 * @see io.hermes.util.gnu.trove.TLinkable
 */
...
public class TLinkedList<T extends TLinkable> extends AbstractSequentialList<T>
  ...
  /**
   * Inserts <tt>linkable</tt> at index <tt>index</tt> in the list. All values > index are shifted
   * over one position to accommodate the new addition.
   *
   * @param index    an <code>int</code> value
  ...
  public void clear() {
    if (null != _head) {
      for (TLinkable link = _head.getNext(); link != null; link = link.getNext()) {
  ...
  /**
   * Copies the list's contents into a native array. This will be a shallow copy: the Tlinkable
   * instances in the Object[] array have links to one another: changing those will put this list
   * into an unpredictable state. Holding a reference to one element in the list will prevent the
   * others from being garbage collected unless you clear the next/previous links. <b>Caveat
   * programmer!</b>
   *
   ...
   * list will be emptied after the copy (as if clear() had been invoked). The Object[] array
   * returned will contain TLinkables that do <b>not</b> hold references to one another and so are
   * less likely to be the cause of memory leaks.
   *
   * @return an <code>Object[]</code> value
   */
   ...
   * <ol>
   * <li>It's really not recommended that the methods implemented by TLinkable be called directly
   * since they're used internally by this class.</li>
   * <li>This solves problems arising from generics when working with the linked objects
   * directly.</li>
   * </ol>
   * <p/>
   * NOTE: this should only be used with nodes contained in the list. The results are undefined with
   * anything else.
   */
  ...
  /**
   * Removes the specified element from the list. Note that it is the caller's responsibility to
   * ensure that the element does, in fact, belong to this list and not another instance of
   * TLinkedList.
   *
  ...
  /**
   * Inserts newElement into the list immediately before current. All elements to the right of and
   * including current are shifted over.
   *
   * @param current    a <code>TLinkable</code> value currently in the list.
   * @param newElement a <code>TLinkable</code> value to be added to the list.
   */
  ...
  /**
   * Inserts newElement into the list immediately after current. All elements to the left of and
   * including current are shifted over.
   *
   * @param current    a <code>TLinkable</code> value currently in the list.
   * @param newElement a <code>TLinkable</code> value to be added to the list.
  ...
  protected final class IteratorImpl implements ListIterator<T> {
    ...
    /**
     * returns the index of the next node in the list (the one that would be returned by a call to
     * next()).
     *
     * @return an <code>int</code> value
     */
     ...
     *
     * @throws IllegalStateException neither next nor previous have been invoked, or remove or add
