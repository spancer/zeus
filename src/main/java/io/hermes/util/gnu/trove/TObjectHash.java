 ...
 */
abstract public class TObjectHash<T> extends THash implements TObjectHashingStrategy<T> {
  ...
  /**
   * Creates a new <code>TObjectHash</code> instance with the default capacity and load factor.
   */
  public TObjectHash() {
    super();
  ...
  /**
   * Creates a new <code>TObjectHash</code> instance with the default capacity and load factor and a
  ...
  /**
   * Creates a new <code>TObjectHash</code> instance whose capacity is the next highest prime above
  ...
  /**
   * Creates a new <code>TObjectHash</code> instance whose capacity is the next highest prime above
   * <tt>initialCapacity + 1</tt> unless that value is already prime. Uses the specified custom
   * hashing strategy.
   *
   * @param initialCapacity an <code>int</code> value
   * @param strategy        used to compute hash codes and to compare objects.
  ...
  /**
   * Creates a new <code>TObjectHash</code> instance with a prime value at or near the specified
  ...
  /**
   * Creates a new <code>TObjectHash</code> instance with a prime value at or near the specified
   * capacity and load factor. Uses the specified custom hashing strategy.
   *
   * @param initialCapacity used to find a prime capacity for the table.
   * @param loadFactor      used to calculate the threshold over which rehashing takes place.
   * @param strategy        used to compute hash codes and to compare objects.
  ...
  /**
   * Locates the index at which <tt>obj</tt> can be inserted. if there is already a value
   * equal()ing
   * <tt>obj</tt> in the set, returns that value's index as <tt>-index - 1</tt>.
   *
   * @param obj an <code>Object</code> value
   * @return the index of a FREE slot at which obj can be inserted or, if obj is already stored in
   * the hash, the negative value of that index, minus 1: -index -1.
   */
  ...
  /**
   * This is the default implementation of TObjectHashingStrategy: it delegates equality comparisons
   * to the first parameter's equals() method.
   *
   ...
   * objects employed as keys. We have to throw an IllegalArgumentException with a rather verbose
   * message telling the user that they need to fix their object implementation to conform to the
  ...
  protected final void throwObjectContractViolation(Object o1, Object o2)
    ...
    throw new IllegalArgumentException("Equal objects must have equal hashcodes. "
        ...
        + "java.lang.Object.hashCode().  See bullet point two " + "in that method's documentation. "
        + "object #1 =" + o1 + "; object #2 =" + o2);
  }
