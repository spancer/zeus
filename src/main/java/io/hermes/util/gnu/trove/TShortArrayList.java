...
public class TShortArrayList implements Externalizable, Cloneable {
  ...
  /**
   * Creates a new <code>TShortArrayList</code> instance with the default capacity.
   */
  ...
  /**
   * Creates a new <code>TShortArrayList</code> instance with the specified capacity.
   *
  ...
  /**
   * Creates a new <code>TShortArrayList</code> instance whose capacity is the greater of the length
   * of <tt>values</tt> and DEFAULT_CAPACITY and whose initial contents are the specified values.
   *
   ...
   * the array shorts on each resize unless <tt>capacity</tt> requires more than twice the current
   * capacity.
   *
   * @param capacity an <code>int</code> value
   */
  ...
  /**
   * Inserts the array of <tt>values</tt> into the list at <tt>offset</tt>. All values including and
   * to the right of <tt>offset</tt> are shifted to the right.
   *
   * @param offset an <code>int</code> value
   * @param values an <code>short[]</code> value
   */
  ...
  /**
   * Replace the values in the list starting at <tt>offset</tt> with <tt>length</tt> values from
   * the
   * <tt>values</tt> array, starting at valOffset.
   *
   * @param offset    the first offset to replace
   * @param values    the source of the new values
   ...
   * Sets the size of the list to 0, but does not change its capacity. This method can be used as an
   * alternative to the {@link #clear clear} method if you want to recyle a list without allocating
   * new backing arrays. This method differs from {@link #reset reset} in that it does not clear the
   * old values in the backing array. Thus, it is possible for {@link #getQuick getQuick} to return
   * stale data if this method is used and the caller is careless about bounds checking.
   *
   * @see #reset
   * @see #clear
   * @see #getQuick
   ...
   * @return true if other is a TShortArrayList and has exactly the same values.
   */
   ...
   * @return the absolute offset in the list of the value, or its negative insertion point into the
   * sorted list.
   */
  ...
  /**
   * Performs a binary search for <tt>value</tt> in the specified range. Note that you
   * <b>must</b> @{link #sort sort} the list or the range before doing a search.
   *
   * @param value     the value to search for
  ...
  /**
   * Searches the list back to front for the last index of <tt>value</tt>, starting at
   * <tt>offset</tt>.
   *
  ...
  /**
   * Searches the list for values satisfying <tt>condition</tt> in the manner of the *nix
   * <tt>grep</tt> utility.
   *
  ...
  /**
   * Searches the list for values which do <b>not</b> satisfy <tt>condition</tt>. This is akin to
   * *nix <code>grep -v</code>.
   *
   * @param condition a condition to apply to each element in the list
