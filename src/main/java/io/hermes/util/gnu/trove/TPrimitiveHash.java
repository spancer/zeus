 ...
 * open-addressed table. So, we have to resort to using a parallel `bookkeeping' array of bytes, in
 * which flags can be set to indicate that a particular slot in the hash table is FREE, FULL, or
 * REMOVED.
 * <p/>
...
abstract public class TPrimitiveHash extends THash {
   ...
   * specified capacity and with the default load factor.
   *
  ...
  /**
   * Creates a new <code>TPrimitiveHash</code> instance with a prime capacity at or near the minimum
   * needed to hold <tt>initialCapacity<tt> elements with load factor
  ...
  /**
   * Returns the capacity of the hash table. This is the true physical capacity, without adjusting
   * for the load factor.
   *
