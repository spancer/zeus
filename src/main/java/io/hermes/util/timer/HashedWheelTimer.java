...
import io.hermes.util.MapBackedSet;
import io.hermes.util.ReusableIterator;
import io.hermes.util.ThreadRenamingRunnable;
import io.hermes.util.concurrent.ConcurrentIdentityHashMap;
 ...
 * <p>
 * You can increase or decrease the accuracy of the execution timing by specifying smaller or larger
 * tick duration in the constructor. In most network applications, I/O timeout does not need to be
 * accurate. Therefore, the default tick duration is 100 milliseconds and you will not need to try
 ...
 * Varghese</a> and Tony Lauck's paper,
 * <a href="http://www-cse.ucsd.edu/users/varghese/PAPERS/twheel.ps.Z">'Hashed and Hierarchical
 * Timing Wheels: data structures to efficiently implement a timer facility'</a>. More comprehensive
 * slides are located
...
public class HashedWheelTimer implements Timer {
   ...
   */
  public HashedWheelTimer(HermesLogger logger, ThreadFactory threadFactory, long tickDuration,
      TimeUnit unit) {
  ...
  public HashedWheelTimer(HermesLogger logger, ThreadFactory threadFactory, long tickDuration,
    ...
    if (activeInstances >= MISUSE_WARNING_THRESHOLD
      ...
      logger.debug("There are too many active " + HashedWheelTimer.class.getSimpleName()
          + " instances (" + activeInstances + ") - you should share the small number "
  ...
  public Timeout newTimeout(TimerTask task, long delay, TimeUnit unit) {
    ...
    lock.readLock().lock();
    try {
      timeout = new HashedWheelTimeout(task, deadline, (int) (wheelCursor + relativeIndex & mask),
          remainingRounds);
