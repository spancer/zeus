...
import io.hermes.action.FailedNodeException;
import io.hermes.action.NoSuchNodeException;
 ...
 * @author spancer.ray
 */
public abstract class TransportNodesOperationAction<Request extends NodesOperationRequest, Response extends NodesOperationResponse, NodeRequest extends NodeOperationRequest, NodeResponse extends NodeOperationResponse> extends
  ...
  @Inject
  public TransportNodesOperationAction(Settings settings, ClusterName clusterName,
  ...
  protected abstract NodeResponse nodeOperation(NodeRequest request) throws HermesException;
  ...
  private class AsyncAction {
    ...
    private AsyncAction(Request request, ActionListener<Response> listener) {
      ...
      if (nodesIds == null || nodesIds.length == 0 || (nodesIds.length == 1 && nodesIds[0]
          .equals("_all"))) {
        int index = 0;
        ...
        if (nodeId.equals("_local") || nodeId.equals(clusterState.nodes().localNodeId())) {
          threadPool.execute(new Runnable() {
            ...
            public void run() {
              try {
                onOperation(
                    nodeOperation(newNodeRequest(clusterState.nodes().localNodeId(), request)));
              } catch (Exception e) {
          ...
          threadPool.execute(new Runnable() {
            ...
            public void run() {
              try {
                onOperation(
                    nodeOperation(newNodeRequest(clusterState.nodes().masterNodeId(), request)));
              } catch (Exception e) {
          ...
          if (node == null) {
            ...
            transportService.sendRequest(node, transportNodeAction(), nodeRequest,
