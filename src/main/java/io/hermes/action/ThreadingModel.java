...
public enum ThreadingModel {
  ...
  public static ThreadingModel fromId(byte id) {
    if (id == 0) {
      ...
      throw new HermesIllegalArgumentException("No threading model for [" + id + "]");
    }
  }
   ...
   * on a different thread than the calling thread (assuming it will be executed on the same node).
   */
   ...
   * on a different thread (than the calling thread or an "expensive" thread, like the IO thread).
   */
