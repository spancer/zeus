...
public class DiscoveryService extends AbstractLifecycleComponent<DiscoveryService> {
  ...
  protected void doStart() throws HermesException {
    ...
    try {
      discovery.start();
      try {
        logger.trace("Waiting for {} for the initial state to be set by the discovery",
            initialStateTimeout);
        if (latch.await(initialStateTimeout.millis(), TimeUnit.MILLISECONDS)) {
          ...
          logger.warn("Waited for {} and no initial state was set by the discovery",
              initialStateTimeout);
        }
  ...
  /**
   * Returns <tt>true</tt> if the initial state was received within the timeout waiting for it on
   * {@link #doStart()}.
   */
